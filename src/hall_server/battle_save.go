...
func (b *BattleSaveManager) Init() {
	b.saves = dbc.BattleSaves
}

func (b *BattleSaveManager) SaveNew(attacker_id, defenser_id int32, data []byte, is_win int32, add_score int32) bool {
...
func (b *BattleSaveManager) GetRecord(requester_id, record_id int32) (attacker_id, defenser_id int32, record_data []byte, save_time int32, is_win int32, add_score int32) {
	row := b.saves.GetRow(record_id)
	if row == nil {
		return
	}
...
func (b *BattleSaveManager) DeleteRecord(requester_id, record_id int32) int32 {
...
func (b *Player) push_battle_record(record_id int32) (pushed bool, remove_record int32) {
	// 未上线的玩家battle_record_list很可能为空，直接返回
	if b.battle_record_list == nil {
		return
	}
	rt, o := b.db.BattleSaves.GetSaveTime(record_id)
	if !o {
		return
	}
	...
	i := int32(0)
	insert := false
	for ; i < b.battle_record_count; i++ {
		var st int32
		st, o = b.db.BattleSaves.GetSaveTime(b.battle_record_list[i])
		if !o {
			for j := i; j < b.battle_record_count-1; j++ {
				b.battle_record_list[j] = b.battle_record_list[j+1]
			}
			b.battle_record_count -= 1
		}
		if rt > st {
			insert = true
			break
	...
	if insert {
		if b.battle_record_count >= max_count {
			battle_record_mgr.DeleteRecord(b.Id, b.battle_record_list[max_count-1])
			remove_record = b.battle_record_list[max_count-1]
		}
		// 往后挪
		for j := b.battle_record_count - 1; j >= i; j-- {
			if j >= max_count-1 {
				continue
			}
			b.battle_record_list[j+1] = b.battle_record_list[j]
		}
		b.battle_record_list[i] = record_id
		if b.battle_record_count < max_count {
			b.battle_record_count += 1
		}
		pushed = true
		...
		if i < max_count {
			...
			battle_record_mgr.DeleteRecord(b.Id, record_id)
		}
	}
	return
}

func (b *Player) init_battle_record_list() {
	if b.battle_record_list != nil {
		return
	}

	max_count := global_config.PlayerBattleRecordMaxCount
	b.battle_record_list = make([]int32, max_count)
	record_ids := b.db.BattleSaves.GetAllIndex()
	if record_ids == nil {
		return
	}
	for i := 0; i < len(record_ids); i++ {
		...
		if row == nil {
			battle_record_mgr.DeleteRecord(b.Id, record_ids[i])
		}
		b.push_battle_record(record_ids[i])
	}

	record_ids = b.db.BattleSaves.GetAllIndex()
	if record_ids != nil {
		log.Debug("++++++++++++++++++++++ Player[%v] inited battle record list: %v", b.Id, b.battle_record_list)
	}
}

func (b *Player) GetBattleRecordList() int32 {
	var record_list []*msg_client_message.BattleRecordData
	if b.battle_record_list != nil {
		for i := 0; i < len(b.battle_record_list); i++ {
			row := dbc.BattleSaves.GetRow(b.battle_record_list[i])
			if row != nil {
	...
	b.Send(uint16(msg_client_message_id.MSGID_S2C_BATTLE_RECORD_LIST_RESPONSE), response)

	return 1
}

func (b *Player) GetBattleRecord(record_id int32) int32 {
	attacker_id, defenser_id, record_data, record_time, _, _ := battle_record_mgr.GetRecord(b.Id, record_id)
	if attacker_id == 0 {
		return int32(msg_client_message.E_ERR_PLAYER_BATTLE_RECORD_NOT_FOUND)
	}
	...
	b.Send(uint16(msg_client_message_id.MSGID_S2C_BATTLE_RECORD_RESPONSE), response)
