...
func (c *ConnTimerWheel) Init() {
	...
	c.op_chan = make(chan *conn_op_data, config.MaxClientConnections)
}

func (c *ConnTimerWheel) Insert(player_id int32) {
	c.op_chan <- &conn_op_data{
		op:        1,
		player_id: player_id,
	}
}

func (c *ConnTimerWheel) Remove(player_id int32) {
	c.op_chan <- &conn_op_data{
		op:        2,
		player_id: player_id,
	}
...
func (c *ConnTimerWheel) insert(player_id int32) bool {
...
func (c *ConnTimerWheel) _remove(p *ConnTimerPlayer) {
	p.timer_list.remove(p.timer)
	delete(c.players, p.player_id)
}

func (c *ConnTimerWheel) remove(player_id int32) bool {
	p := c.players[player_id]
	if p == nil {
		return false
	}
...
func (c *ConnTimerWheel) Run() {
	...
	for {
			...
			case d, ok := <-c.op_chan:
				{
					...
					if d.op == 1 {
						c.insert(d.player_id)
					} else if d.op == 2 {
						c.remove(d.player_id)
					}
				}
			default:
				{
		...
		curr_num := int32(len(c.players))
		last_num := atomic.LoadInt32(&c.last_players_num)
		if curr_num != last_num {
			atomic.StoreInt32(&c.last_players_num, curr_num)
...
func (c *ConnTimerWheel) GetCurrPlayerNum() int32 {
	return atomic.LoadInt32(&c.last_players_num)
}
