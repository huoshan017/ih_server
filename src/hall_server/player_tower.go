...
func (p *Player) send_tower_data(send bool) int32 {
	need_level := system_unlock_table_mgr.GetUnlockLevel("TowerEnterLevel")
	if need_level > p.db.Info.GetLvl() {
		log.Error("Player[%v] level not enough level %v enter tower", p.Id, need_level)
		return int32(msg_client_message.E_ERR_PLAYER_LEVEL_NOT_ENOUGH)
	}

	var tower_keys, remain_seconds int32
	var updated bool
	updated, tower_keys, remain_seconds = p.check_tower_keys()
	if send || updated {
		response := &msg_client_message.S2CTowerDataResponse{
			CurrTowerId:   p.db.TowerCommon.GetCurrId(),
			TowerKeys:     tower_keys,
			RemainSeconds: remain_seconds,
		}
		p.Send(uint16(msg_client_message_id.MSGID_S2C_TOWER_DATA_RESPONSE), response)
		log.Debug("Player[%v] tower data %v", p.Id, response)
	}
	return 1
}

func (p *Player) check_tower_keys() (is_update bool, keys int32, next_remain_seconds int32) {
	tower_key_max := global_config.TowerKeyMax
	tower_key_get_interval := global_config.TowerKeyGetInterval
	//keys = p.db.TowerCommon.GetKeys()
	keys = p.get_resource(global_config.TowerKeyId)
	old_keys := keys
	if keys < tower_key_max {
		now_time := int32(time.Now().Unix())
		last_time := p.db.TowerCommon.GetLastGetNewKeyTime()
		if last_time == 0 {
			keys = global_config.TowerKeyMax
			last_time = now_time
			p.db.TowerCommon.SetLastGetNewKeyTime(now_time)
			...
			if keys_num > 0 {
				...
				p.db.TowerCommon.SetLastGetNewKeyTime(now_time - y)
			}
			if keys < tower_key_max {
	...
	if old_keys != keys {
		p.set_resource(global_config.TowerKeyId, keys)
		is_update = true
	}
	return
}

func (p *Player) check_and_send_tower_data() {
	p.send_tower_data(false)
}

func (p *Player) fight_tower(tower_id int32) int32 {
	// 是否时当前层的下一层
	var tower *table_config.XmlTowerItem
	curr_id := p.db.TowerCommon.GetCurrId()
	if curr_id == 0 {
		tower = tower_table_mgr.Get(tower_id)
	} else {
		if curr_id >= tower_id {
			log.Error("Player[%v] already fight tower[%v]", p.Id, tower_id)
	...
	_, keys, _ := p.check_tower_keys()
	if keys <= 0 {
		log.Error("Player[%v] fight tower not enough key", p.Id)
		return int32(msg_client_message.E_ERR_PLAYER_TOWER_NOT_ENOUGH_STAMINA)
	}

	err, is_win, my_team, target_team, my_artifact_id, target_artifact_id, enter_reports, rounds, _ := p.FightInStage(3, stage, nil, nil)
	if err < 0 {
		log.Error("Player[%v] fight tower %v failed, team is empty", p.Id, tower_id)
		return err
	}

	//p.db.TowerCommon.SetKeys(keys - 1)
	p.add_resource(global_config.TowerKeyId, -1)
	tower_key_max := global_config.TowerKeyMax
	if keys >= tower_key_max {
		p.db.TowerCommon.SetLastGetNewKeyTime(int32(time.Now().Unix()))
	}
	member_damages := p.tower_team.common_data.members_damage
	member_cures := p.tower_team.common_data.members_cure
	response := &msg_client_message.S2CBattleResultResponse{
		...
		Rounds:              rounds,
		MyMemberDamages:     member_damages[p.tower_team.side],
		TargetMemberDamages: member_damages[p.target_stage_team.side],
		MyMemberCures:       member_cures[p.tower_team.side],
	...
	data := p.Send(uint16(msg_client_message_id.MSGID_S2C_BATTLE_RESULT_RESPONSE), response)

	if is_win {
		p.db.TowerCommon.SetCurrId(tower_id)
		now_time := int32(time.Now().Unix())
		p.db.TowerCommon.SetPassTowerTime(now_time)
		// 名次
		p.tower_update_rank(tower_id, now_time)
		// 奖励
		p.send_stage_reward(stage.RewardList, 3, 0)
		// 录像
		for i := int32(1); i <= 3; i++ {
	...
	log.Trace("Player %v fight tower %v", p.Id, tower_id)

	return 1
}

func (p *Player) get_tower_records_info(tower_id int32) int32 {
	...
	p.Send(uint16(msg_client_message_id.MSGID_S2C_TOWER_RECORDS_INFO_RESPONSE), response)
	return 1
}

func (p *Player) get_tower_record_data(tower_fight_id int32) int32 {
	row := dbc.TowerFightSaves.GetRow(tower_fight_id)
	if row == nil {
	...
	p.Send(uint16(msg_client_message_id.MSGID_S2C_TOWER_RECORD_DATA_RESPONSE), response)

	return 1
}
func (p *Player) tower_update_rank(tower_id, update_time int32) {
...
func (p *Player) load_tower_db_data() {
	tower_id := p.db.TowerCommon.GetCurrId()
	if tower_id <= 0 {
		return
	}
	update_time := p.db.TowerCommon.GetPassTowerTime()
	if update_time == 0 {
		update_time = p.db.Info.GetLastLogin()
	}
	p.tower_update_rank(tower_id, update_time)
}
