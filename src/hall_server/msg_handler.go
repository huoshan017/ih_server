...
func (h *MsgHttpHandle) ServeHTTP(w http.ResponseWriter, r *http.Request) {
...
func (h *MsgHandlerMgr) Init() bool {
	h.msgid2handler = make(map[int32]*MsgHandlerInfo)
	return true
}

func (h *MsgHandlerMgr) SetMsgHandler(msg_code uint16, msg_handler CLIENT_MSG_HANDLER) {
	h.msgid2handler[int32(msg_code)] = &MsgHandlerInfo{msg_handler: msg_handler, if_player_msg: false}
}

func (h *MsgHandlerMgr) SetPlayerMsgHandler(msg_code uint16, msg_handler CLIENT_PLAYER_MSG_HANDLER) {
	h.msgid2handler[int32(msg_code)] = &MsgHandlerInfo{player_msg_handler: msg_handler, if_player_msg: true}
}

func (h *MsgHandlerMgr) StartHttp() bool {
	var err error
	h.reg_http_mux()

	h.msg_http_listener, err = net.Listen("tcp", config.ListenClientInIP)
	if nil != err {
		log.Error("Center StartHttp Failed %s", err.Error())
		return false
	}

	signal_mgr.RegCloseFunc("msg_handler_mgr", h.CloseFunc)
	...
	err = msg_http_server.Serve(h.msg_http_listener)
	if err != nil {
...
func (h *MsgHandlerMgr) StartHttps(crt_file, key_file string) bool {
	h.reg_http_mux()

	signal_mgr.RegCloseFunc("msg_handler_mgr", h.CloseFunc)

	h.login_http_server = http.Server{
	...
	err := h.login_http_server.ListenAndServeTLS(crt_file, key_file)
	if err != nil {
		log.Error("启动消息处理服务失败%s", err.Error())
		return false
	}
...
func (h *MsgHandlerMgr) CloseFunc(info *SignalRegRecod) {
	if nil != h.msg_http_listener {
		h.msg_http_listener.Close()
	}

	h.login_http_server.Close()
...
func (h *MsgHandlerMgr) reg_http_mux() {
