...
func (h *HallServer) Init() (ok bool) {
	h.start_time = time.Now()
	h.shutdown_lock = &sync.Mutex{}
	...
	// rpc初始化
	if !h.init_rpc_service() {
		return
	}
	if !h.init_rpc_client() {
		return
	}
...
func (h *HallServer) OnInit() (err error) {
...
func (h *HallServer) Start(use_https bool) (err error) {
	log.Event("服务器已启动", nil, log.Property{Name: "IP", Value: config.ListenClientInIP})
	log.Trace("**************************************************")
...
func (h *HallServer) Run() {
	defer func() {
		...
		h.shutdown_completed = true
	}()

	h.ticker = timer.NewTickTimer(1000)
	h.ticker.Start()
	defer h.ticker.Stop()

	go h.redis_conn.Run(1000)
	if USE_CONN_TIMER_WHEEL == 0 {
...
func (h *HallServer) Shutdown() {
	if !h.initialized {
		return
	}

	h.shutdown_lock.Lock()
	defer h.shutdown_lock.Unlock()
...
func (h *HallServer) OnTick(t timer.TickTime) {
	player_mgr.OnTick()
}

func (h *HallServer) OnAccept(c *socket.TcpConn) {
	log.Info("HallServer OnAccept [%s]", c.GetAddr())
}

func (h *HallServer) OnConnect(c *socket.TcpConn) {

}

func (h *HallServer) OnDisconnect(c *socket.TcpConn, reason socket.E_DISCONNECT_REASON) {
	if c.T > 0 {
...
func (h *HallServer) CloseConnection(c *socket.TcpConn, reason socket.E_DISCONNECT_REASON) {
	if c == nil {
		log.Error("参数为空")
		return
	}
...
func (h *HallServer) OnUpdate(c *socket.TcpConn, t timer.TickTime) {
