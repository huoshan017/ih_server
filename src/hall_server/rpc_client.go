...
func (s *HallServer) init_rpc_client() bool {
	// 注册用户自定义RPC数据类型
	rpc_proto.RegisterRpcUserType()

	s.rpc_client = rpc.NewClient()
	var on_connect rpc.OnConnectFunc = func(args interface{}) {
		rpc_client := args.(*rpc.Client)
		proc_string := "H2R_ListenRPCProc.Do"
		var arg = rpc_proto.H2R_ListenIPNoitfy{ListenIP: config.ListenRpcServerIP, ServerId: config.ServerId}
	...
	s.rpc_client.SetOnConnect(on_connect)

	if !s.rpc_client.Dial(config.RpcServerIP) {
...
func (s *HallServer) uninit_rpc_client() {
	if s.rpc_client != nil {
		s.rpc_client.Close()
		s.rpc_client = nil
	}
...
/*
// 游戏服到游戏服调用
func (s *HallServer) rpc_hall2hall(receive_player_id int32, method string, args interface{}, reply interface{}) error {
	if s.rpc_client == nil {
	...
	err := s.rpc_client.Call("H2H_CallProc.Do", transfer_args, transfer_reply)
	if err != nil {
