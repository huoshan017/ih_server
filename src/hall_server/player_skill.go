...
// 血最少选择
func skill_get_hp_min_targets(_ int32, target_team *BattleTeam, skill_data *table_config.XmlSkillItem) (pos []int32) {
...
// 随机一个目标
func _random_one_target(_ int32, target_team *BattleTeam, except_pos []int32) (pos int32) {
	pos = int32(-1)
	c := int32(0)
...
// 神器伤害效果
func skill_effect_artifact_damage(_, target *TeamMember, effect []int32) (target_damage int32, is_absorb bool) {
...
func (list *BuffList) remove_buff(buff *Buff) bool {
	if list.buffs == nil || list.buffs[buff] == nil {
		return false
	}
	if buff.prev != nil {
	...
	if list.owner != nil {
		list.owner.remove_buff_effect(buff)
	}

	buff_pool.Put(buff)
	delete(list.buffs, buff)

	return true
}

// 战报删除BUFF
func (list *BuffList) add_remove_buff_report(buff_id int32) {
	report := list.owner.team.GetLastReport()
	if report != nil {
		buff := msg_battle_buff_item_pool.Get()
		buff.Pos = list.owner.pos
		buff.BuffId = buff_id
		buff.Side = list.owner.team.side
		report.RemoveBuffs = append(report.RemoveBuffs, buff)
	}
}

// 免疫次数
func (list *BuffList) cost_resist_num(buff *Buff) {
	// 免疫次数
	if buff.resist_num > 0 {
		if buff.resist_num > 1 {
			...
			b := buff.buff
			list.remove_buff(buff)
			list.add_remove_buff_report(b.Id)
			log.Debug("Team[%v] member[%v] BUFF[%v]类型免疫次数[%v]用完", list.owner.team.side, list.owner.pos, b.Id, buff.buff.ResistCountMax)
		}
	}
}

// 检测BUFF互斥或免疫
func (list *BuffList) check_buff_mutex(b *table_config.XmlStatusItem) bool {
	hh := list.head
	for hh != nil {
		next := hh.next
		// 免疫类型
		for j := 0; j < len(hh.buff.ResistMutexTypes); j++ {
			if b.MutexType > 0 && b.MutexType == hh.buff.ResistMutexTypes[j] {
				list.cost_resist_num(hh)
				log.Debug("Team[%v] member[%v] BUFF[%v]类型[%v]排斥BUFF[%v]类型[%v]", list.owner.team.side, list.owner.pos, hh.buff.Id, hh.buff.MutexType, b.Id, b.MutexType)
				return true
			}
		}
		// 免疫BUFF ID
		for j := 0; j < len(hh.buff.ResistMutexIDs); j++ {
			if b.MutexType > 0 && b.Id == hh.buff.ResistMutexIDs[j] {
				list.cost_resist_num(hh)
				log.Debug("Team[%v] member[%v] BUFF[%v]排斥BUFF[%v]", list.owner.team.side, list.owner.pos, hh.buff.Id, b.Id)
				return true
			}
		}
		...
		for j := 0; j < len(b.CancelMutexTypes); j++ {
			if hh.buff.MutexType > 0 && hh.buff.MutexType == b.CancelMutexTypes[j] {
				list.remove_buff(hh)
				list.add_remove_buff_report(hh.buff.Id)
				log.Debug("Team[%v] member[%v] BUFF[%v]类型[%v]驱散了BUFF[%v]类型[%v]", list.owner.team.side, list.owner.pos, b.Id, b.CancelMutexTypes[j], hh.buff.Id, hh.buff.MutexType)
			}
		}
		// 驱散BUFF ID
		for j := 0; j < len(b.CancelMutexIDs); j++ {
			if hh.buff.MutexType > 0 && hh.buff.Id == b.CancelMutexIDs[j] {
				list.remove_buff(hh)
				list.add_remove_buff_report(hh.buff.Id)
				log.Debug("Team[%v] member[%v] BUFF[%v]驱散了BUFF[%v]", list.owner.team.side, list.owner.pos, b.Id, hh.buff.Id)
			}
		}
		hh = next
	}
...
func (list *BuffList) add_buff(attacker *TeamMember, b *table_config.XmlStatusItem, skill_effect []int32) (buff_id int32) {
	buff := buff_pool.Get()
	buff.clear()

	if list.buffs == nil {
		list.buffs = make(map[*Buff]*Buff)
	}
	if _, o := list.buffs[buff]; o {
		return
	}
	...
	list.tail = buff
	buff.next = nil
	buff.team_side = list.owner.team.side
	buff.owner_pos = list.owner.pos
	list.buffs[buff] = buff
	buff_id = b.Id

	if list.owner != nil {
		list.owner.add_buff_effect(buff, skill_effect)
	}

	return
}

func (list *BuffList) on_round_end() {
	if list.owner.is_will_dead() || list.owner.is_dead() {
		return
	}

	var item *msg_client_message.BattleFighter
	members_damage := list.owner.team.common_data.members_damage
	members_cure := list.owner.team.common_data.members_cure
	bf := list.head
	for bf != nil {
		next := bf.next
		if bf.round_num > 0 {
			if bf.buff.Effect[0] == BUFF_EFFECT_TYPE_DAMAGE {
				dmg := buff_effect_damage(bf.attack, bf.dmg_add, bf.param, bf.buff.Effect[1], list.owner)
				if dmg != 0 {
					list.owner.add_hp(-dmg)
					if list.owner.is_will_dead() {
						list.owner.on_will_dead(bf.attacker)
					}
					if list.owner.is_will_dead() {
						// 有死亡后触发的被动技
						if list.owner.has_trigger_event([]int32{EVENT_AFTER_TARGET_DEAD}) {
							list.owner.on_after_will_dead(bf.attacker)
						}
						if list.owner.is_will_dead() {
							// 延迟被动技有没有死亡后触发
							if !bf.attacker.team.HasDelayTriggerEventSkill(EVENT_AFTER_TARGET_DEAD, list.owner) {
								list.owner.set_dead(bf.attacker, nil)
							}
						}
					}
					// --------------------------- 战报 ---------------------------
					if !list.owner.team.is_sweeping {
						// 血量变化的成员
						if item == nil {
							item = list.owner.build_battle_fighter(0)
							item.Side = list.owner.team.side
							list.owner.team.common_data.changed_fighters = append(list.owner.team.common_data.changed_fighters, item)
						}
						item.Damage += dmg
						if dmg != 0 {
							if bf.attacker != nil {
					...
					// ------------------------------------------------------------
					log.Debug("Team[%v] member[%v] hp damage[%v] on buff[%v] left round[%v] end", list.owner.team.side, list.owner.pos, dmg, bf.buff.Id, bf.round_num)
				}
			}
			...
			if bf.round_num <= 0 {
				...
				if !list.owner.team.is_sweeping {
					...
					b.BuffId = buff_id
					b.Pos = list.owner.pos
					b.Side = list.owner.team.side
					list.owner.team.common_data.remove_buffs = append(list.owner.team.common_data.remove_buffs, b)
				}
				// ------------------------------------------------------------
				log.Debug("Team[%v] member[%v] buff[%v] round over", list.owner.team.side, list.owner.pos, buff_id)
			}
		}
		bf = next
	}

	list.owner.team.DelaySkillEffect()

	if item != nil {
		item.HP = list.owner.hp
	}
}

// 状态伤害效果
func buff_effect_damage(user_attack, _, skill_damage_coeff, attr int32, target *TeamMember) (damage int32) {
