...
func (t *TeamMember) build_battle_fighter(damage int32) *msg_client_message.BattleFighter {
	item := msg_battle_fighter_pool.Get()
	item.Side = t.team.side
	item.Pos = t.pos
	item.Damage = damage
	item.HP = t.hp
	item.MaxHP = t.attrs[ATTR_HP_MAX]
	item.Energy = t.energy
	item.Shield = t.attrs[ATTR_SHIELD]
	return item
}

func (t *TeamMember) build_battle_member() *msg_client_message.BattleMemberItem {
	mem := msg_battle_member_item_pool.Get()
	mem.Side = t.team.side
	mem.Id = t.id
	mem.TableId = t.card.Id
	mem.Rank = t.card.Rank
	mem.Level = t.level
	mem.Pos = t.pos
	mem.HP = t.hp
	mem.MaxHP = t.attrs[ATTR_HP_MAX]
	mem.Energy = t.energy
	return mem
}
