...
func (conn *HallConnection) Send(msg_id uint16, msg proto.Message) {
	...
	if conn.use_https {
		...
		resp, err = client.Post(conn.hall_ip+"/client_msg", "application/x-www-form-urlencoded", bytes.NewReader(data))
	} else {
		resp, err = http.Post(conn.hall_ip+"/client_msg", "application/x-www-form-urlencoded", bytes.NewReader(data))
	}
	if nil != err {
		log.Error("login C2S_MSG http post[%s] error[%s]", conn.hall_ip+"/client_msg", err.Error())
		return
	}
	...
	for _, m := range S2C_MSG.MsgList {
		...
		for cur_len < total_data_len {
			...
			log.Trace("玩家[%d:%s]收到服务器返回%v:[%s]", conn.playerid, conn.acc, msg_code, new_msg.String())
...
func (mgr *MsgHandlerMgr) Init() bool {
	mgr.msgid2handler = make(map[int32]CLIENT_MSG_HANDLER)
	mgr.RegisterMsgHandler()
	return true
}

func (mgr *MsgHandlerMgr) SetMsgHandler(msg_code uint16, msg_handler CLIENT_MSG_HANDLER) {
	log.Info("set msg [%d] handler !", msg_code)
	mgr.msgid2handler[int32(msg_code)] = msg_handler
}

func (mgr *MsgHandlerMgr) RegisterMsgHandler() {
	mgr.SetMsgHandler(uint16(msg_client_message_id.MSGID_S2C_ENTER_GAME_RESPONSE), S2CEnterGameHandler)
	mgr.SetMsgHandler(uint16(msg_client_message_id.MSGID_S2C_BATTLE_RESULT_RESPONSE), S2CBattleResultHandler)
}
