...
func (proc *G2G_CommonProc) Get(arg *rpc_proto.G2G_GetRequest, result *rpc_proto.G2G_GetResponse) (err error) {
	...
	if rpc_client == nil {
		return fmt.Errorf("!!!!!! Not found rpc client by object type %v object id %v", arg.ObjectType, arg.ObjectId)
	}
...
func (proc *G2G_CommonProc) MultiGet(arg *rpc_proto.G2G_MultiGetRequest, result *rpc_proto.G2G_MultiGetResponse) (err error) {
	defer func() {
		if e := recover(); e != nil {
			log.Stack(e)
		}
	...
	for sid, objects := range sid2objects {
		...
		if rpc_client == nil {
			return fmt.Errorf("!!!!!! Not found rpc client by server id %v", sid)
		}
...
func (proc *G2G_CommonProc) BroadcastGet(arg *rpc_proto.G2G_BroadcastGetRequest, result *rpc_proto.G2G_BroadcastGetResponse) (err error) {
	defer func() {
		if e := recover(); e != nil {
			log.Stack(e)
		}
