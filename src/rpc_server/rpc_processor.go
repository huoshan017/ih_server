...
func (proc *H2H_CallProc) Do(args *rpc_proto.H2R_Transfer, reply *rpc_proto.H2R_TransferResult) error {
	defer func() {
...
func (proc *H2R_ListenRPCProc) Do(args *rpc_proto.H2R_ListenIPNoitfy, result *rpc_proto.H2R_ListenIPResult) error {
	defer func() {
		if err := recover(); err != nil {
			log.Stack(err)
		}
...
func (proc *H2R_GlobalProc) ChargeSave(args *rpc_proto.H2R_ChargeSave, result *rpc_proto.H2R_ChargeSaveResult) error {
	defer func() {
		if err := recover(); err != nil {
			log.Error(err)
		}
...
// 初始化
func (proc *RpcServer) init_proc_service() bool {
	proc.rpc_service = &rpc.Service{}

	if !proc.rpc_service.Register(&H2H_CallProc{}) {
		return false
	}

	if !proc.rpc_service.Register(&H2R_ListenRPCProc{}) {
		return false
	}

	if !proc.rpc_service.Register(&H2R_GlobalProc{}) {
		return false
	}
	...
	return proc.rpc_service.Listen(config.ListenIP) == nil
}

// 反初始化
func (proc *RpcServer) uninit_proc_service() {
