...
func (this *TcpConn) Send(msgid uint16, msg proto.Message) {
	if msg == nil {
	...
	log.Debug("发送[%v][玩家%v][%v][%v][%v][%v字节][%v字节] %v ",
		this.addr, this.T, length, this.bytes_sended, "{{"+msg.String()+"}}")

	this.send_data(msgid, data)
}
...
func (this *TcpConn) on_recv(d []byte) {
	...
	if err != nil {
		log.Error("[%v][玩家%v]反序列化失败 %v data:%v err%s", this.addr, this.T, this.node.get_message_name(type_id), data, err.Error())
		this.event_disc(E_DISCONNECT_REASON_PACKET_MALFORMED)
		return
	}
